...
//-------------------------------------------------------------------
const cmdList = ["set", "noise", "shape", "shoot", "up", "distance", "age", "delete", "mode"];
...
async function matchCmd(cmd, sender) {
    ...
    if (cmd[0] == "help" || cmd[0] == "h" || cmd[0] == "?") {
        system.run(() => {
            ...
            overworld.runCommandAsync("tellraw " + sender.name + " {\"rawtext\":[{\"text\":\"§2---Main Command list---\"}]}");
            for (let i = 0; i < cmdList.length; i++) {
                overworld.runCommandAsync("tellraw " + sender.name + " {\"rawtext\":[{\"text\":\"§7" + (i + 1) + "> §r" + cmdList[i] + "\"}]}");
            }
        });
    } else if (cmd[0] == "pos") {
        system.run(() => {
        ...
        system.run(() => {
            ...
            sel1[1].addScore(sender, Number(cmd[1]));
            sel2[1].addScore(sender, Number(cmd[1]));
        });
    }
        ...
        world.sendMessage(" " + JSON.parse("1+1"));
    }
}
